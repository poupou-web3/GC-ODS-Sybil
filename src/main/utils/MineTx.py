...
class MineTx:
    ...
    def get_df_txn(self, address, tx_type):
        ...
        while per_page == self.per_page_txn:
            ...
            if tx_type == "normal":
                txn_dict = self.eth.get_normal_txs_by_address_paginated(address=address, startblock=0, endblock=99999999, sort='desc', page=page, offset=self.per_page_txn)
                self.api_counter += 1
            else: #transaction are considered internal
                txn_dict = self.eth.get_internal_txs_by_address_paginated(address=address, startblock=0, endblock=99999999, sort='desc', page=page, offset=self.per_page_txn)
                self.api_counter += 1
            if page == 0:
                list_txn = txn_dict
            else:
    ...
    def create_csv(self, csv_path, address, tx_type, network):
        try:
            ...
            print("No transaction found for address #{0} {1} /n error: {2}".format(self.api_counter, address, err))
